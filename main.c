...
int main(void)
{
  ...
  while (1)
  {
    /* USER CODE END WHILE */
	  ...
	  char buffer[16]; //create buffer
	  sprintf(buffer, "ADC: %d",adc_val);
	  writeLCD(buffer); //Print to LCD


	  HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_7);

	    // Delay according to delay_t value
	  HAL_Delay(delay_t);
    /* USER CODE BEGIN 3
  }
  /* USER CODE END 3 */
...
void EXTI0_1_IRQHandler(void)
{
  ...
  {
	  //Set the static lastTick variable
...
void writeLCD(char *char_in)
{
    ...
    lcd_command(CLEAR);
    // Write the input string to the LCD screen
